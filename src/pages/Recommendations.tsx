...
export default function Recommendations() {
  ...
  const copy = (text: string, label: string) => {
    navigator.clipboard.writeText(text).then(() => toast.success(`${label} ${t("copied")}`));
  };
  ...
  const startApply = (edition: SavedEdition) => {
    ...
    if (!recId && data) {
      ...
      toast.message(t("Search saved so we can re-apply it later"));
    }
    if (!recId) return;
  ...
  return (
    <div className="min-h-screen">
      <PageHeader
        eyebrow={t("Smart Recommendations")}
        title=""
        titleMain={t("Editions across the")}
        titleEmphasis={t("tongues of the world")}
        subtitle={t("Type a book, however roughly. We identify it, then surface the best editions in English, Arabic, French, and German — with ISBN and publisher — plus AI-powered companion tools.")}
      />
      ...
      <div className="px-4 sm:px-8 lg:px-14 py-8 max-w-5xl">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={t("e.g. crime and punishment, the prophet kahlil, sapiens harari…")}
          ...
          <section className="mt-8 luxury-panel rounded-sm p-4">
            <div className="flex items-baseline justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display text-sm tracking-wider text-foreground">{t("Saved recommendations")}</h3>
              ...
              {recs.map((r) => (
                ...
                >
                  <button
                    ...
                    title={`${r.editions.length} ${t("editions")} · ${t("saved")} ${new Date(r.savedAt).toLocaleDateString()}`}
                  >
                  ...
                  <button
                    ...
                    aria-label={t("Remove saved search")}
                  >
        ...
        {loading && (
          <div className="mt-12 text-center">
            ...
            <p className="font-serif italic text-muted-foreground mt-3">{t("Identifying the book and gathering its editions…")}</p>
          </div>
        )}
          ...
          <div className="mt-10 space-y-10 animate-fade-in">
            {data.detected && (
              <div className="luxury-panel rounded-sm p-5">
                <div className="flex items-baseline justify-between gap-4 flex-wrap">
                  <div>
                    <p className="mono text-[0.55rem] tracking-[0.3em] uppercase text-muted-foreground">{t("Identified")}</p>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="mono text-[0.6rem] tracking-[0.25em] uppercase text-primary">
                      {(data.detected.confidence * 100).toFixed(0)}% {t("confidence")}
                    </span>
                    <Button
                      onClick={saveCurrent}
                      variant="outline"
                    ...
                    >
                      {isCurrentSaved
                        ? <><BookmarkCheck className="h-3.5 w-3.5 mr-2" /> {t("Saved")}</>
                        : <><Bookmark className="h-3.5 w-3.5 mr-2" /> {t("Save search")}</>}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <section>
              <div className="flex items-baseline gap-3 mb-4">
                ...
                <h3 className="font-display text-xl text-foreground">{t("Best edition per language")}</h3>
                <div className="flex-1 h-px bg-border/60" />
              </div>
              {data.editions.length === 0 ? (
                <p className="font-serif italic text-muted-foreground">{t("No editions surfaced. Try a more specific title.")}</p>
              ) : (
                    ...
                    <article key={e.language} className="luxury-panel rounded-sm p-4 flex gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <dl className="mt-3 space-y-1 mono text-[0.6rem] tracking-wide text-muted-foreground">
                          {e.publisher && <div><span className="text-foreground/80">{t("Publisher")}:</span> {e.publisher}{e.publishedDate ? ` · ${e.publishedDate.slice(0, 4)}` : ""}</div>}
                          {e.pageCount && <div><span className="text-foreground/80">{t("Pages")}:</span> {e.pageCount}</div>}
                          {e.isbn13 && (
                            <div className="flex items-center gap-1">
                              <span className="text-foreground/80">ISBN-13:</span> {e.isbn13}
                              <button onClick={() => copy(e.isbn13!, "ISBN-13")} className="text-primary hover:text-primary-glow" aria-label={t("Copy") + " ISBN-13"}><Copy className="h-3 w-3" /></button>
                            </div>
                          )}
                          {e.isbn10 && !e.isbn13 && (
                            <div className="flex items-center gap-1">
                              <span className="text-foreground/80">ISBN-10:</span> {e.isbn10}
                              <button onClick={() => copy(e.isbn10!, "ISBN-10")} className="text-primary hover:text-primary-glow" aria-label={t("Copy") + " ISBN-10"}><Copy className="h-3 w-3" /></button>
                            </div>
                          )}
                        </dl>
                        <div className="mt-3 flex flex-wrap gap-3 items-center text-xs">
                          ...
                          >
                            {t("Apply to a book")} <ArrowRight className="h-3 w-3" />
                          </button>
                        </div>
                      </div>
                    </article>
            ...
            {data.tools.length > 0 && (
              <section>
                <div className="flex items-baseline gap-3 mb-4">
                  ...
                  <h3 className="font-display text-xl text-foreground">{t("AI-powered companion tools")}</h3>
                  <div className="flex-1 h-px bg-border/60" />
                </div>
                  ...
                  {data.tools.map((tool, i) => (
                    <div key={i} className="luxury-panel rounded-sm p-4">
                      <div className="flex items-baseline justify-between gap-2">
                        <h4 className="font-display text-foreground">{tool.name}</h4>
                        <span className="mono text-[0.55rem] tracking-[0.25em] uppercase text-primary">{tool.category}</span>
                      </div>
                      <p className="font-serif italic text-sm text-muted-foreground mt-2">{tool.why}</p>
                      {tool.url && (
                        <a href={tool.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs mono text-primary hover:text-primary-glow tracking-wider">
        ...
        {!loading && !data && recs.length === 0 && (
          <p className="mt-10 font-serif italic text-muted-foreground">
            {t("Try “Sapiens”, “الكيمياء باولو كويلو”, “Le Petit Prince”, or just an author name.")}
          </p>
        )}
      </div>
      ...
      {picker && (
        ...
        >
          ...
          >
            <div className="flex items-baseline justify-between mb-3">
              <div>
                <p className="mono text-[0.55rem] tracking-[0.25em] uppercase text-muted-foreground">{t("Apply edition")}</p>
              ...
              <button onClick={() => setPicker(null)} className="text-muted-foreground hover:text-foreground" aria-label={t("Close")}>
                <X className="h-4 w-4" />
              </button>
            </div>

            <p className="font-serif italic text-sm text-muted-foreground mb-3">
              {t("Choose which Book Brain entry should receive this edition's language, ISBN, cover, and a first-underlined prompt.")}
            </p>
            ...
            <div className="overflow-y-auto -mx-2 px-2 space-y-1">
              {books.length === 0 && (
                <p className="font-serif italic text-muted-foreground text-sm py-6 text-center">
                  {t("Your shelf is empty. Add a book first, then return here to apply the edition.")}
                </p>
              )}
              {books.map((b) => (
                <button
